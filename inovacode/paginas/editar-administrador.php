...
<body>
  ...
  <div class="container d-flex justify-content-center mt-5">
    ...
    <div class="card p-4">
     ...
     $C =conectar();
     $consulta="select * from administrador where id_admin = ".$v." ";
     
      $res=$C->query($consulta);

     while($p=$res->fetch_assoc()){
    ?>
      <!--formulario para dar de alte el administrador nuevo-->
      <form action="../acciones/actualizar-administrador.php" method="POST">

        
        <input  name="id" value="<?php echo $p['id_admin'];  ?>" type="hidden" >
        


        <div class="mb-3 text-start">
          <label for="user" class="form-label fw-bold">Usuario</label>
          <input  name="u" type="text" class="form-control" id="user" placeholder="" value="<?php echo $p['usuario'];  ?>" required>
        </div>

        <div class="mb-3 text-start">
          <label name="correo" for="password" class="form-label fw-bold">Correo electronico</label>
          <input name="e" type="email" class="form-control" id="correo" placeholder="example@gmail.com" value="<?php echo $p['correo'];  ?>" required>
        </div>


        <div class="mb-3 text-start">
          <label name="contrasena" for="password" class="form-label fw-bold">Contraseña</label>
          <input name="p" type="password" class="form-control" id="contrasena" placeholder="********" value="<?php echo $p['contrasena'];  ?>" required>
        </div>
